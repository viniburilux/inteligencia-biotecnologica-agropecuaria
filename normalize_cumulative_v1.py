...
RAW = ROOT / "raw_v0"
OUT = ROOT / "normalized_v1"
MANIFESTS = [ROOT / "ingestion_manifest_v0.jsonl", ROOT / "ingestion_manifest_v1.jsonl"]
...
def main():
    ...
    write_jsonl(OUT / "works_v1.jsonl", works)
    write_jsonl(OUT / "actors_v1.jsonl", actors)
    write_jsonl(OUT / "institutions_v1.jsonl", institutions)
    write_jsonl(OUT / "relations_v1.jsonl", relations)
    write_jsonl(OUT / "application_signals_v1.jsonl", signals)
    summary = {
    ...
    (OUT / "normalization_summary_v1.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
