...
RAW = ROOT / "raw_v0"
OUT = ROOT / "normalized_v2"
MANIFESTS = [ROOT / "ingestion_manifest_v0.jsonl", ROOT / "ingestion_manifest_v1.jsonl", ROOT / "ingestion_manifest_v2.jsonl"]
    ...
    for manifest_path in MANIFESTS:
        ...
        for line in manifest_path.read_text(encoding="utf-8").splitlines():
            if not line.strip():
                continue
            row = json.loads(line)
            row["_manifest_path"] = manifest_path.name
            if not row.get("saved_path") and row.get("raw_path"):
                row["saved_path"] = row["raw_path"]
            if not row.get("saved_sha256") and row.get("raw_sha256"):
                row["saved_sha256"] = row["raw_sha256"]
            rows.append(row)
    return rows
    ...
    for entry in manifest:
        ...
        for item in payload.get("results", []) or []:
            ...
            work = {
                ...
                "status": "captured",
                "provenance": {"manifest_path": entry.get("_manifest_path", "ingestion_manifest_v0.jsonl"), "raw_path": entry.get("saved_path"), "raw_sha256": entry.get("saved_sha256")},
            }
            works.append(work)
            if app_terms:
...
def google_patents_rows(manifest):
    ...
    for entry in manifest:
        ...
        for line in path.read_text(encoding="utf-8").splitlines():
            ...
            provenance = {
                "manifest_path": entry.get("_manifest_path", "ingestion_manifest_v0.jsonl"),
...
def main():
    ...
    write_jsonl(OUT / "works_v2.jsonl", works)
    write_jsonl(OUT / "actors_v2.jsonl", actors)
    write_jsonl(OUT / "institutions_v2.jsonl", institutions)
    write_jsonl(OUT / "relations_v2.jsonl", relations)
    write_jsonl(OUT / "application_signals_v2.jsonl", signals)
    summary = {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "metadata_only": True,
        "source_manifests": [path.name for path in MANIFESTS if path.exists()],
    ...
    (OUT / "normalization_summary_v2.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
