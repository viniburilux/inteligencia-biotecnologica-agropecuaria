...
QUERY_FILE = ROOT / "EXPANSION_QUERIES_V1.json"
RAW_DIR = ROOT / "raw_v1" / "openalex"
MANIFEST = ROOT / "ingestion_manifest_v1.jsonl"
SUMMARY = ROOT / "expansion_openalex_summary_v1.json"
...
with MANIFEST.open("a", encoding="utf-8") as mf:
    for q in queries:
        ...
        raw_path = RAW_DIR / f"{safe_id}_{q['query'][:50].lower().replace(' ', '_').replace('/', '_')}.json"
        ...
        entry = {
            ...
            "query": q["query"],
            "source": "openalex",
            "source_url": r.url if 'r' in locals() and hasattr(r, "url") else url,
...
summary = {
    ...
    "queries": results,
    "notes": ["No full texts or scientific data were downloaded.", "The expansion manifest is separate from the V0/V0-second-round manifest."]
}
